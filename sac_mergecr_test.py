...
def save_models(model, weights_folder = 'sac_merge_att/weights'):
...
# env1 = sector_cr_v0.SectorCR_ATT(render_mode='human')
env2 = merge_v0.MergeEnv_ATT(render_mode='human')

# env_choice = np.random.randint(0,1)
# if env_choice == 0:
#      env = env1
# else:
#      env = env2
...
model.actor.load_state_dict(torch.load('sac_merge_att/weights/actor.pt'))
...
for episode in range(num_episodes):
    # env_choice = np.random.randint(0,2)
    # if env_choice == 0:
    #     env = env1
    # else:
    env = env2
